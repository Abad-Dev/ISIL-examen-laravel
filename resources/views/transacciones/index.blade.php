...
@section('content')
    <div class="space-y-6">
        ...
        @else
            <x-transacciones.filters
                :cuentas="$cuentas"
                :categorias="$categorias"
                :filters="$filters"
                :expanded="$filtersActive || $errors->hasAny(['fecha_desde', 'fecha_hasta', 'cuenta_id', 'categoria_id', 'tipo', 'monto_min', 'monto_max'])"
            />
            ...
            @if ($transacciones->isEmpty())
                <div class="rounded-2xl border border-dashed border-slate-200 bg-white/60 px-6 py-12 text-center dark:border-slate-700 dark:bg-slate-900/40">
                    ...
                    <p class="mt-4 text-sm text-slate-500 dark:text-slate-400">
                        ...
                        @else
                            {{ __('You have no transactions yet. Record your first movement.') }}
                        @endif
                    </p>
                </div>
            @else
    ...
    @if ($cuentas->isNotEmpty())
        <x-transacciones.form-modal
            :cuentas="$cuentas"
            :categorias="$categorias"
            :open="$errors->isNotEmpty() && ! $errors->hasAny(['fecha_desde', 'fecha_hasta', 'cuenta_id', 'categoria_id', 'tipo', 'monto_min', 'monto_max'])"
